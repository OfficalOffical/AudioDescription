...
public class TFLiteObjectDetectionAPIModel implements Detector {
  ...
  public List<Recognition> recognizeImage(final Bitmap bitmap) {
    ...
    // Log.i("WOLOLO",isFitForVoiceOver(recognitions.get(0)));
    // integer icin log.d ("tag", ""+integerdegiskeni)
    // Log.d ("tag", ""+locationOnScreen(recognitions.get(0)));
    speakFunction( isFitForVoiceOver(recognitions.get(0)), locationOnScreen(recognitions.get(0)) ,isClose(recognitions.get(0)));
  ...
  @Override
  public void speakFunction(String sound, String location, boolean isClose) {
    //True  == yakın False == Uzak

    String normalizeUzaklik;

    if(isClose)
      normalizeUzaklik = " close";
    else
      normalizeUzaklik = " far";
    ...
    tts=new TextToSpeech(context, new TextToSpeech.OnInitListener() {
      ...
      public void onInit(int status) {
        if (status != TextToSpeech.ERROR) {
          ...
          counter++;
          if ((sound != "") && (counter % 20 == 0))
            tts.speak(sound+location+ normalizeUzaklik,TextToSpeech.QUEUE_FLUSH,null);
  ...
  @Override
  public String isFitForVoiceOver(Recognition recognition) {

    if (recognition.getConfidence() > 0.70)
      return recognition.getTitle();
    else
      return "";
  }

  public boolean isClose(Recognition recognition) {

    int a = (int)( recognition.getLocation().height() * recognition.getLocation().width());
  ...
  public String locationOnScreen(Recognition recognition){

    int location = (int) ( (recognition.getLocation().right - recognition.getLocation().left) /2 );

    if (recognition.getLocation().width() > 200)
      return " Very";
    else if (recognition.getLocation().right - (recognition.getLocation().width()/2) < 115)
      return " Left";
    else if((300 - (recognition.getLocation().left+(recognition.getLocation().width()/2)) ) < 100)
      return " Right";
    else
      return " Middle";
